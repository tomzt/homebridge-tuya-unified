...
// Note: upstream also wires an AdaptiveLightingController here, gated by a
// per-device `adaptiveLighting` config override. This plugin doesn't have a
// deviceOverrides config layer yet (see NOTES.md), so adaptive lighting is
// not ported — it's a roadmap item, not a silently-broken feature.
